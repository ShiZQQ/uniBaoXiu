          ...
          <view class="section__content" :class="{'section__content--visible': item.show}">
            ...
            <view class="section__content__btns">
              <view v-for="(section_btn,section_index) in item.optional" :key="section_index"
                class="section__content__btns__item" :class="[`tn-main-gradient-${tuniaoColorList[index]}--light`]" 
                @click="sectionBtnClick(index, section_index)">
                <view class="section__content__btns__item__bg"
                  :class="[`tn-main-gradient-${tuniaoColorList[index]}`, {'section__content__btns__item__bg--active': getSectionIndexValue(modeIndex, index) === section_index}]"></view>
                <view class="section__content__btns__item--text tn-text-ellipsis"
                  :class="[getSectionIndexValue(modeIndex, index) === section_index ? 'section__content__btns__item--text--active' : `tn-color-${tuniaoColorList[index]}`]">{{ section_btn }}</view>
              </view>
            </view>
          </view>
        </block>
...
<script>
  export default {
    ...
    computed: {
      ...
      haveTips() {
        return Array.isArray(this.tipsData) && this.tipsData.length > 0
      },
      multiMode() {
        return Array.isArray(this.sectionList) && this.sectionList.length > 1
      },
      sectionModeList() {
      ...
      // 兼容处理vuex_custom_bar_height
      vuex_custom_bar_height() {
        return Number(this.customBarHeight) || 0
      }
    },
    data() {
      return {
        // 图鸟颜色列表（替换未定义的$t.color.getTuniaoColorList()）
        tuniaoColorList: ['primary', 'success', 'warning', 'danger', 'info'],
    ...
    methods: {
      // 安全获取sectionIndex值（解决空值访问问题）
      getSectionIndexValue(modeIndex, index) {
        if (!Array.isArray(this.sectionIndex) || 
            !Array.isArray(this.sectionIndex[modeIndex]) || 
            !this.sectionIndex[modeIndex][index]) {
          return 0
        }
        return this.sectionIndex[modeIndex][index].value || 0
      },
      
      // 深拷贝工具函数（替换未定义的this.$t.deepClone）
      deepClone(obj) {
          ...
          success: (systemInfo) => {
            ...
            if (this.multiMode) {
              uni.createSelectorQuery().in(this).select('.mode-switch').boundingClientRect(data => {
                ...
                if (data.bottom >= (systemInfo.safeArea?.height || systemInfo.windowHeight)) {
                  this.sectionScrollFlag = false
                } else {
                  this.sectionScrollFlag = true
                  const containerBaseHeight = (systemInfo.safeArea?.height || systemInfo.windowHeight) - data.bottom
                  this.sectionScrollViewStyle.height = (containerBaseHeight - navBarHeight) + (systemInfo.statusBarHeight || 0) - uni.upx2px(75) + 'px'
                }
              }).exec()
            } else {
              if (!this.noDemo) {
      ...
      getModeTabsInfo() {
        ...
        const listLength = Array.isArray(this.sectionModeListInfos) ? this.sectionModeListInfos.length : 0
        ...
        view.exec(res => {
          ...
          // 将每个模式的宽度放入list中
          res.map((item, index) => {
            if (item && this.sectionModeListInfos[index]) {
              this.sectionModeListInfos[index].width = item.width || 0
            }
          })
          
          // 初始化滑块的宽度
          if (this.sectionModeListInfos[0]) {
            this.modeSwitchSliderStyle.width = (this.sectionModeListInfos[0].width || 0) + 'px'
          }
      ...
      switchMode(index) {
        ...
        // 边界检查
        if (index < 0 || index >= (this.sectionModeListInfos.length || 0)) return
      ...
      initSectionBtns() {
        ...
        // 安全处理sectionList
        const safeSectionList = this.sectionList || []
        
        this.sectionIndex = safeSectionList.map((item) => {
          if (item && item.hasOwnProperty('section') && Array.isArray(item.section) && item.section.length > 0) {
        ...
        // 使用自定义深拷贝替换this.$t.deepClone
        this._sectionList = this.deepClone(safeSectionList)
        
        // 给本地选项按钮列表给默认show属性
        this._sectionList.map((item) => {
          if (item && Array.isArray(item.section)) {
      ...
      updateSectionBtns(sectionIndex = -1, showState = true) {
        // 安全获取section数据
        const currentSection = this._sectionList[this.modeIndex]
        let sectionOptional = currentSection?.section || []
        
        this.btnsList = sectionOptional.map((item, index) => {
          if (!item) return {}
          
          // 安全获取sectionIndex值
          const sectionIndexData = this.sectionIndex[this.modeIndex]?.[index] || {value: 0, change: false}
          let changeValue = sectionIndexData.change || false
          let currentSectionIndexValue = sectionIndexData.value || 0
          ...
          // 处理最大最小值（修复边界错误）
          const optionalLength = Array.isArray(item.optional) ? item.optional.length : 0
          if (indexValue < 0) {
            indexValue = 0
          }
          if (indexValue >= optionalLength) {
            indexValue = optionalLength > 0 ? optionalLength - 1 : 0 // 修复：改为length-1，避免越界
          }
          
          // 安全设置值
          if (!this.sectionIndex[this.modeIndex]) {
            this.sectionIndex[this.modeIndex] = []
          }
      ...
      updateSectionBtnsState(sectionIndex = -1, showState = true) {
        // 替换this.$t.array.isArray为原生Array.isArray
        if (Array.isArray(sectionIndex)) {
          if (sectionIndex.length === 0) {
            return
          }
          
          const safeSectionList = this.sectionList[this.modeIndex]?.section || []
          sectionIndex = sectionIndex.filter((item) => item >= 0 && item < safeSectionList.length)
          
          sectionIndex.map((item) => {
            if (this.btnsList[item]) {
              this.btnsList[item]['show'] = showState
            }
            if (this._sectionList[this.modeIndex]?.section?.[item]) {
              this._sectionList[this.modeIndex].section[item]['show'] = showState
            }
          })
        } else {
          const safeSectionList = this.sectionList[this.modeIndex]?.section || []
          if (sectionIndex < 0 || sectionIndex >= safeSectionList.length) {
            return
          }
          
          // 将按键的对应显示状态设置为对应的状态
          if (this.btnsList[sectionIndex]) {
            this.btnsList[sectionIndex]['show'] = showState
          }
          if (this._sectionList[this.modeIndex]?.section?.[sectionIndex]) {
      ...
      updateSectionBtnsValue(modeIndex = 0, sectionIndex = -1, value = 0) {
        const safeSectionList = this.sectionList[modeIndex]?.section || []
        if (sectionIndex < 0 || sectionIndex >= safeSectionList.length) {
          return
        }
        
        // 安全初始化数组
        if (!this.sectionIndex[modeIndex]) {
          this.sectionIndex[modeIndex] = []
        }
      ...
      sectionBtnClick(index, sectionIndex) {
        ...
        if (!this.sectionIndex[this.modeIndex][index]) {
          this.sectionIndex[this.modeIndex][index] = {value: 0, change: false}
        }
...
<style lang="scss" scoped>
  .dynamic-demo {
    ...
    .demo-container {
      ...
      background-color: #FFFFFF;
      box-shadow: 0rpx 10rpx 50rpx 0rpx rgba(0, 3, 72, 0.1);
      margin: -190rpx 30rpx 5rpx 30rpx;
      border-radius: 20rpx;
      position: relative;
      display: flex;
      justify-content: center;
